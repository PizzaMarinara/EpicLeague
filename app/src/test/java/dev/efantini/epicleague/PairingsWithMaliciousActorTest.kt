...
class PairingsWithMaliciousActorTest {

    var torneiList = mutableListOf<Tournament>()

    private val AVERAGE_PLAYER_NUMBER = 10 // At least 4!
    private val BIG_TOURNAMENT_COEFFICIENT = 3
    private var BIG_TOURNAMENT_WINNER_ID by Delegates.notNull<Int>()
    private val MAX_PLAYERS = AVERAGE_PLAYER_NUMBER + (AVERAGE_PLAYER_NUMBER).floorDiv(2) + 1
    private val TOURNAMENT_NUMBER = 12

    @Before
    fun setUp() {

        val torneoBig = Tournament(name = "Just a test Tournament BIG!").apply {
            leaguePointsAssigned = "10-8-6-5-2-2-2-2-1"
            pointsToLast = true
        }
        torneoBig.also {
            val actualPlayerNumber = AVERAGE_PLAYER_NUMBER * BIG_TOURNAMENT_COEFFICIENT
        ...
        while (!torneoBig.isEnded()) {
            torneoBig.pairNewRound()
            torneoBig.tournamentRounds.last().tournamentMatches.forEach {
            ...
            torneoBig.tournamentRounds.last().isFinished = true
            // printStandingsShort(torneoBig)
        }
        BIG_TOURNAMENT_WINNER_ID = torneoBig.getStandings().first().id.toInt()
        torneiList.add(torneoBig)

        for (j in 2..TOURNAMENT_NUMBER) {
            ...
            torneo.also {
                ...
                for (k in 1..MAX_PLAYERS) {
                    // Big tournament winner does not come back
                    if (k != BIG_TOURNAMENT_WINNER_ID)
                        playerIdsList.add(k)
                }
                for (k in 1..actualPlayerNumber) {
                    playerIdsList.shuffle()
    ...
    fun `rounds are paired correctly`() {
        ...
        println("BIG TOURNAMENT WINNER WAS: $BIG_TOURNAMENT_WINNER_ID")
        // printChampionshipPoints(torneiList, true)
        // printChampionshipPoints(torneiList, false)
    }
    ...
    private fun printChampionshipPoints(
        ...
        if (useIncenter) {
            ...
            torneiList.filter { it.isEnded() }.forEach {
                rankingsLists.add(
                    calculateStandardPointsForRankings(
                        it.getStandings(),
    ...
    private fun <T> calculateStandardPointsForRankings(
        elements: List<T>,
